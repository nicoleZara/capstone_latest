...
def edit_profile(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            # Save the updated user profile to SQLite
            form.save()

            # Update the MongoDB user profile
            mongo_user = MongoDBUser.find_by_username(request.user.username)
            if mongo_user:
                # Update MongoDB fields using the form data
                mongo_user['fname'] = form.cleaned_data['first_name']
                mongo_user['lname'] = form.cleaned_data['last_name']
                mongo_user['gender'] = form.cleaned_data['gender']
                mongo_user['birthday'] = form.cleaned_data['birthday']
                mongo_user['age'] = form.cleaned_data['age']
                mongo_user['purpose'] = form.cleaned_data['purpose']

                # Update the MongoDB document
                user_collection.update_one({'username': request.user.username}, {'$set': mongo_user})
